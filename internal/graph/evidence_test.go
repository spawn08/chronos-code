...
// evidenceFixture indexes a small Go workspace and returns its
// codebase_context tool. The scope does not watch: files changed afterwards
// stay at their indexed hash, which the staleness checks rely on.
func evidenceFixture(t *testing.T, extra map[string]string) (*IndexScope, string) {
	t.Helper()
...
func evidenceCall(t *testing.T, s *IndexScope, args map[string]any) *evidenceResult {
	t.Helper()
	result, err := toolFrom(t, s.Tools(), "codebase_context").Handler(context.Background(), args)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEvidenceOneTurnCrossFile(t *testing.T) {
	s, _ := evidenceFixture(t, nil)
	before := s.Engine().Status().Generation
	def := toolFrom(t, s.Tools(), "codebase_context")
	// Separate sessions: within one session, source already delivered is
	// collapsed to a signature.
	result, err := def.Handler(storage.WithSession(context.Background(), "first"), map[string]any{"query": "Charge", "max_tokens": 4096})
	if err != nil {
		t.Fatal(err)
	}
	...
	secondResult, err := def.Handler(storage.WithSession(context.Background(), "second"), map[string]any{"query": "Charge", "max_tokens": 4096})
	if err != nil {
		t.Fatal(err)
	}
	...
	if after := s.Engine().Status().Generation; after != before {
		t.Fatalf("read-only tool changed the index generation: %d -> %d", before, after)
	}
	ranked := evidenceCall(t, s, map[string]any{"query": "payment", "include": []any{"definitions"}})
	if len(ranked.Items) == 0 || ranked.Items[0].Name != "Charge" || ranked.Items[0].IndexedHash == "" {
		t.Fatalf("doc-text query lost definition/provenance: %+v", ranked)
	}
}
...
func TestEvidenceMetadataOnlyBudgetAndConservativeFallback(t *testing.T) {
	// Thirty definitions of Charge with long packages and signatures: their
	// metadata alone cannot fit 256 tokens.
	extra := map[string]string{}
	params := strings.Repeat("signature int, ", 60)
	for i := 0; i < 30; i++ {
		dir := fmt.Sprintf("%s%02d", strings.Repeat("metadata", 12), i)
		extra[dir+"/charge.go"] = fmt.Sprintf("package p%02d\nfunc Charge(%sz int) {}\n", i, params)
	}
	s, _ := evidenceFixture(t, extra)
	result := evidenceCall(t, s, map[string]any{"query": "Charge", "include": []any{"definitions"}, "max_tokens": 256})
	...
	if result.IOBytes != 0 || !result.Truncated || len(result.Omitted) == 0 {
...
func TestEvidenceFullSerializedBudget(t *testing.T) {
	...
	for _, budget := range []int{256, 384, 512, 1024, 4096} {
		got := evidenceCall(t, s, map[string]any{"query": "Charge", "ranges": []any{map[string]any{"file": "service.go", "start_line": 2, "end_line": 3}}, "max_tokens": budget})
...
func TestEvidenceBoundedReadsContainmentAndStaleness(t *testing.T) {
	...
	got := evidenceCall(t, s, map[string]any{"ranges": []any{map[string]any{"file": "large.go", "start_line": 1, "end_line": 1}}, "include": []any{"excerpts"}})
	...
	deep := evidenceCall(t, s, map[string]any{"ranges": []any{map[string]any{"file": "large.go", "start_line": 999999, "end_line": 1000000}}})
	...
	aggregate := evidenceCall(t, s, map[string]any{"ranges": spans})
	...
	inside := evidenceCall(t, s, map[string]any{"ranges": []any{map[string]any{"file": "inside.go", "start_line": 1, "end_line": 1}}})
	...
	unterminated := evidenceCall(t, s, map[string]any{"ranges": []any{map[string]any{"file": "unterminated.go", "start_line": 1, "end_line": 1}}})
	...
	blocked := evidenceCall(t, s, map[string]any{"ranges": []any{map[string]any{"file": "escape.go", "start_line": 1, "end_line": 1}}})
	...
	stale := evidenceCall(t, s, map[string]any{"symbols": []any{"Charge"}})
	...
	missing := evidenceCall(t, s, map[string]any{"symbols": []any{"Charge"}})
...
func TestEvidenceValidationCancellationAndBatch(t *testing.T) {
	...
	got := evidenceCall(t, s, map[string]any{"symbols": []any{"Charge", "Handle", "NotIndexed"}, "include": []any{"definitions"}, "max_tokens": 2048})
