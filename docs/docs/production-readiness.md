...
| P0 | Complex-task routing | `Orchestrator.Execute` can switch qualifying enabled requests to the read-only `delivery-strategist` | Complex implementation can terminate with a frontier proposal instead of a patch; rolling replanning is not implemented | Keep the legacy-named `ppd` policy shadowed by default until successive frontiers are admitted and returned to the primary loop |
